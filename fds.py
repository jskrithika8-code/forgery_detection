...
# Initialize EasyOCR reader (GPU disabled by default)
try:
...
def preprocess(img: np.ndarray) -> np.ndarray:
    """
    Convert to grayscale, blur, and adaptive threshold to improve OCR.
...
def process_document_with_documentai(gcs_input_uri: str, mime_type: str) -> str:
    """
    Send document in GCS to Document AI and return extracted text.
    """
...
def detect_forgery_rules(extracted_text: str, processed_images: List[str]) -> dict:
    """
    Simple rule-based forgery detection returning a report dict.
    """
    suspicious_sections = []
    flagging_reasons = []
    confidence = 0.0

    # Rule 1: Font mismatch (simulated)
    ...
    # Rule 2: Layout anomaly (simulated)
    suspicious_sections.append({
        "page": 2,
        "type": "layout_anomaly",
...
if uploaded_file is not None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            st.error(f"Error converting PDF or reading image: {e}")
            image_list = []
            ...
            for i, img_cv_page in enumerate(image_list, start=1):
                ...
                if force_engine == "Tesseract":
                    ...
                    if easy_langs and easyocr_reader is not None:
                        ...
                        st.warning("EasyOCR not available or requested languages not supported; falling back to Tesseract.")
                        ocr_engine_used = "None"
